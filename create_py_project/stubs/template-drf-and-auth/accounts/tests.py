...
class UsersManagersTests(TestCase):
    def test_create_user(self):
        ...
        user = User.objects.create_user(
            email="user@email.com", name="user", password="foo"
        )
...
class AccountsTests(TestCase):
    def test_user_login(self):
        ...
        response = self.client.post(
            "/api/v1/token/", {"email": user.email, "password": "foo"}
        )
    ...
    def test_auth_user_register_with_unequal_password_field(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertJSONEqual(
            response.content, {"password": ["Password fields didn't match."]}
        )

    def test_auth_get_user(self):
        user = User.objects.create_user(
            email="normal@user.com", name="normal", password="password@123"
        )
        response = self.client.post(
            "/api/v1/token/", {"email": user.email, "password": "password@123"}
        )
    ...
    def test_update_user_general_setting(self):
        ...
        response = self.client.put(
            "/api/v1/auth/user/generalsetting/",
            data,
            **header,
            content_type="application/json",
        )
